...
в local-first режиме (прогресс только в браузере). С ними — регистрация без email
через anonymous session и синхронизация прогресса.
...
## 3. Настрой Auth без email

1. **Authentication → Sign In / Providers** — включи **Anonymous sign-ins**.
2. **Authentication → Providers → Email** — выключи **Email signups**, если регистрация через email не нужна.
...
Если при регистрации появляется сообщение про anonymous sign-ins, проверь, что эта
возможность включена в Supabase Auth. Письма при такой регистрации не отправляются.
...
Теперь регистрация создаёт anonymous Supabase-пользователя с телефоном, Telegram
и кодом доступа в metadata. Прогресс подтягивается из Supabase, а изменения
сохраняются автоматически (с задержкой ~1 с).
