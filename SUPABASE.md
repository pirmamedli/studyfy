...
Если при регистрации появляется сообщение про некорректный путь запроса, проверь,
что `VITE_SUPABASE_URL` указывает именно на `https://<project-ref>.supabase.co`,
а в Auth URL Configuration разрешены адреса локального приложения.
