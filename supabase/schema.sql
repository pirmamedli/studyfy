...
-- Запусти это в Supabase → SQL Editor. Скрипт идемпотентный.
-- Отличается от старого серверного проекта: одна таблица состояния,
-- доступ строго к своей строке через Row Level Security.
-- ============================================================================

-- Всё пользовательское состояние (UserState приложения) хранится как jsonb.
create table if not exists public.studyfy_state (
  user_id uuid primary key references auth.users(id) on delete cascade,
  state jsonb not null default '{}'::jsonb,
  -- денормализованный никнейм для уникальности между пользователями
